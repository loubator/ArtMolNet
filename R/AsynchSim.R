...
# asynch
  ...
  # do all in 1 loop
  # order the vector
RandSpecie=sample(Species,1)

TotiState<-t(as.data.frame(iStates))
while(!identical(paste(TotiState[nrow(TotiState),],collapse = ""),paste(TotiState[nrow(TotiState)-1,],collapse = ""))|nrow(TotiState)<2){
    
  for(Ospecies in Species){
